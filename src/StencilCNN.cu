...
void stencil_convolution(const double* input, double* output, const double* params, int rows, int cols) {
...
int main(int argc, char* argv[]) {
    ...
    for(int c = 0; c < CHANNELS; c++){
        stencil_convolution(input[c], output_gpu[c], params[c], rows, cols);
    }
